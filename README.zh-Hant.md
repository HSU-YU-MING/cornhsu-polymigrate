...
- 兩者都配不起來的**啟發式建議**:只差分隔符的檔名
  (`2025-light-offering` vs `2025_light_offering`)、共用相簿、
  slug 內日期正規化(`20240121` vs `01212024`)、標題相似度
...
這是刻意的,不是還沒做完——背後的量測、以及要重新考慮的話需要什麼條件,
記在 [docs/hreflang_量測與決策.md](docs/hreflang_量測與決策.md)。
