...
所以 PolyMigrate 把它當成強證據,不當成聖旨:一則宣告要能用於配對,必須不是 `x-default`、
不是指向自己、而且解析後真的在你的鏡像裡(含 host 要一致——不少站的英文版在
`en.example.org`,只比路徑的話那一頁會變成自己的翻譯)。所有宣告——包含不可用的那些——
都會寫進 `hreflang_map.csv`,附 `in_mirror` / `reciprocal` / `usable` 三欄,
