...
**還有一個結構性的疑慮**,量測之外、但方向一致:hreflang 維護良好的站,
通常表示背後有一個稱職的 SEO/開發團隊——而那正好是**最不需要這個工具**的站。
兩個屬性反相關。再者,hreflang 正確**且**路徑對稱的站,對稱路徑配對本來就解決了,
hreflang 加零。所以 hreflang 真正能派上用場的區間,是「宣告正確 **且** 路徑不對稱」
那條窄帶——這條帶子有多寬,目前沒有任何資料可以回答。
